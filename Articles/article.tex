...
\begin{document}
	...
	Let $\mathcal{M}$ be a set of messages of length $c$ over the alphabet $\mathcal{A} = \{1, 2, \hdots, {l \choose a}\}$ (where $a$ is a positive integer) and $m = | \mathcal{M} |$. Messages in $\mathcal{M}$ are stored in a neural network, of $c \cdot l$ units.
	...
	$\tilde{W} = \max_{1\le i \le m} x^i x^{i\intercal}$
...
%		\subsubsection{The "$w$-winners take all" rule}
		...
		Intuitively the choice of $w = a$ is optimal (it it is the most natural with $w = 1$ as well).
		...
		$\mbox{select}(w, v)$ is equal to the $w$-th element of $v$ (counting repetitions).		
