...
\end{scope}
  ...
  edge (noderectanglefill20pt60pt)
  edge (noderectanglefill60pt20pt)
  edge (noderectangledraw60pt20pt)
  edge (nodecirclefill60pt60pt)
  %edge (noderectanglefill40pt40pt)
  edge (noderectangledraw20pt60pt)
  %%%%%%%%%%%
  ...
  edge (noderectangledraw20pt60pt)
  %%%%%%%%%%%%%%
  (nodecirclefill20pt20pt) edge (noderectanglefill20pt60pt)
  edge (noderectangledraw20pt60pt)
  edge (noderectanglefill60pt20pt)
  edge (noderectangledraw60pt20pt)
  %%%%%
  (nodecirclefill60pt60pt) edge (noderectanglefill20pt60pt)
  edge (noderectangledraw60pt20pt)
  edge (noderectanglefill60pt20pt)
  edge (noderectangledraw20pt60pt)
  %%%%%
  (noderectanglefill20pt60pt) edge (noderectangledraw20pt60pt)
  edge (noderectangledraw60pt20pt)
  %%%%%%%%%%%
  (noderectanglefill60pt20pt) edge (noderectangledraw20pt60pt)
  edge (noderectangledraw60pt20pt)
  ;
\end{tikzpicture}
...
%		\end{algorithm}
	...
	We study the properties of the network once some messages have been stored in it. We are interested in how much edges have been added to the network and if the process introduced confusion between messages.	
	...
	\begin{figure}[!htb]
		\includegraphics[width=8.5cm]{Courbes/c5l256densities}%remplacement_densite_c8l256a4}
	...
	The rate $R$ of the network measures how much information is stored in the network compared to what it takes to store the network itself. Note it doesn't take into account how effectively messages are retrieved (if we store too much messages in the network, the proportion of retrieved messages approaches $0$ but the rate is high)
		...
		Since messages are not stored in an ordered fashion, the entropy $H(\mathcal{M})$ of the set of messages is $\log_2(\frac{({\ell \choose a}^c)^M}{M!}) \mathop{=}_{M \rightarrow +\infty} M(c \log_2{\ell \choose a } - \log_2(M) + \frac{1}{\log 2}) + O(\log_2(M)) $ (thanks to  Stirling's formula).% The ratio of information effectively stored in the network is $P_{retrieve} \times  \frac{2M \left(c \log_2{\ell \choose a } - \log_2(M) + \frac{1}{\log 2} \right)}{c(c-1)\ell^2}$.
	...
	We are interested in distinct problems of messages retrieval. We detail what those problems are. Since those problems vary in hardness, we propose distinct measures for efficiencies. Efficiency measures the ratio of information effectively retrieved over the hardness of the problem. It can be expressed in function of the rate $R$, the retrieval rate $P_{retrieve}$ and some measure of hardness of the problem $h$.
	
	To account for hardness of problems, we consider an "optimal" associative memory. We are interested in retrieving messages with a few iterations at most. However an "optimal" memory could store $\mathcal{M}$ with $H(\mathcal{M})$ bits and recover messages by maximum likelihood (with ambiguities causing errors in our study, to simplify the problem). We detail the performance of such a memory for each problem, which gives use a measure for the hardness of each problem.
	...
	We can extend the mapping $f$ to $\left( \mathcal{A} \cup \{ \intercal \}\right)^c$, by associating to the symbol $\intercal$ an "erased cluster" where none of the units are activated. If we consider the binary representation $\mu$ of $m$, erased symbols corresponds to erased clusters. %An erased cluster is a cluster where all units are set to $0$. 
	...
	Notice increasing the size of the alphabet makes the problem easier as confusion
	...
	The problem of resilience takes into account the possibility for the network of sustaining damages. Despite those, the network can still work as an associative memory, albeit with decreased performance. The network $W$ first sustains random binary noises of magnitude $\psi$. The new matrix is called $\tilde{W}$ and satisfies for all $i$ and $i'$ in $[c \cdot \ell ]$, %$W_{ii'} = \big \begin{array}{ccc} W & o\end{array}$
...
%	bc : bad unit in correct cluster
	...
	Correctly activated units in uncorrupted clusters achieve at least a score of $a(c-c_e - 1) + \gamma$. 
	...
	$P(n_{dge} = a(c - c_e) + x) = {a (c_e - 1) \choose x} d^x (1-d)^{a (c_e - 1)-x}$
	...
	Finally $P(n_{dbe} = x) = P(n_{dbc} = x) = {a(c - 1) \choose x} d^x (1-d)^{a(c- 1) -x}$	
...
%	\end{align*}
	...
	\begin{figure}[!htb]
		\includegraphics[width=8.5cm]{Courbes/comp_rulesmax}%thpsi_c8l256e4}
