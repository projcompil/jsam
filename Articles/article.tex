...
\begin{document}
	...
	An associative memory is able to retrieve a message through a partial version of it.

	We first explain how we will store messages. Secondly we detail how messages are retrieved by a multipartite clustered network.
	...
	Messages in $\mathcal{M}$ are stored in a clustered neural network, of $c$ clusters of $l$ units each.
	
	Let $\mathcal{M}$ be a set of messages of length $c$ over a finite alphabet $\mathcal{A} = \{1, 2, \hdots, {l \choose a}\}$ (where $a$ is a positive integer) and $m = | \mathcal{M} |$. 
	
	binary constant weight codes of weight $a$.
	
	States of the network are characterized by which neurons are activated. A message corresponds to one, and only one, state of the network. 
	...
	$\tilde{W} = \max_{x\in \mathcal{M}} x x^{\intercal}$
...
%		\subsubsection{The "$w$-winners take all" rule}
	...
	We now study the performance of the new retrieving rule on different problems of retrieving messages : first partially erased messages, then erroneous messages, finally partially erased messages when the network has been damaged.
