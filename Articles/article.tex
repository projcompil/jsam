...
%		\end{algorithm}
	...
	The rate $R$ of the network measures how much information is stored in the network compared to what it takes to store it. Note it doesn't take into account how effectively messages are retrieved.	
		...
		$R$ can be written $\frac{2H(\mathcal{M})}{c(c-1) \ell^2}$.
		
		A very good approximation is thereby $R = \frac{2M \left(c \log_2{\ell \choose a } - \log_2(M) + \frac{1}{\log 2} \right)}{c(c-1)\ell^2}$.
		
		The efficiency of the network measures how much information can be retrieved compared to how much could be at the maximum. The efficiency is called $\eta$. $\eta = P_{retrieve} R  h^{-1}$. 
	...
	We are interested in distinct problems of messages retrieval. We detail what those problems are. Since those problems vary in hardness, we propose distinct measures for efficiencies. We first compute the rate of the network. Efficiency measures the ratio of information effectively retrieved over the hardness of the problem. It can be expressed in function of the rate $R$, the retrieval rate $P_{retrieve}$ and some measure of hardness of the problem $h$.
