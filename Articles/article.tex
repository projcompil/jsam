...
\begin{document}
	...
	Previous works (\cite{GriBer20114, GriBer20122}) considered the storage of binary constant weight codewords of weight $1$. We study the case of arbitrary weight. It is convenient to take $\mathcal{A}$ as the set of all binary constant weight codewords of weight $a$, where $a$ is a positive integer which we will call number of activities.
	...
	States of the network are characterized by which neurons are activated. A message corresponds to one, and only one, state of the network. A message $y$ of length $c$ can be seen as an array $x$ of length $c\dot l$ where for each $j\in [1,c]$, the $j$-th symbol of $y$ denoted $y_j$ is represented by $x[(j-1)l+1\hdots jl]$.\footnote{This is a clustered representation. $x$ is separated in $c$ clusters of $l$ units.} We say the network is in state $x$ if for all $j\in [1, c]$, cluster $j$ in the network follows the activation pattern $x[(j-1)l+1\hdots jl]$.
	...
	Storing a message $y$ in the network amounts to consider the network in state $x$, that is for all $j$ and $k$ such as $1 \le k \le l$ and $1 \le j \le c$, the unit $k$ in the cluster $j$ is activated if and only if $x[(j-1)l+k] = 1$ ; then adding all connections between activated units excepting for units sharing the same cluster. Thus we obtain a multipartite clique between activated units representing a message.% The parameter $a$ describing the number of activated units per cluster will be called number of activities.
	
	Unit $k$ in cluster $j$ thus corresponds to the index $l = (j-1)c+k$.
	
	%The state of the network corresponds to a message.	
	
	We can describe the network which is an undirected unweighted graph, by its adjacency matrix $W$. As for a message $x$, $x x^{\intercal}$ is the matrix describing the graph where all edges between active units exist, and only them, we can express $W$ in function of the following :
	
	$\tilde{W} = \max_{x\in \mathcal{M}} x x^{\intercal}$ which is the matrix where even edges between units in the same cluster have been added.
  ...
  As taking a maximum is associative and commutative, storing can be done online and is independent of the order of messages.
...
%		\subsubsection{The "$w$-winners take all" rule}
		...
		At each step $t$, we compute the scores $s^{t} = \gamma \tilde{x^{t}} + W \tilde{x^{t}}$
		
		$\mbox{select}(w, v)$ is equal to the $w$-th greatest element of $v$ (counting repetitions).\footnote{$\mbox{select}(3, [3,2,1,2,0]) = 2$}
		
		$\tilde{x}^{t+1}_{(j-1)c+k} = \bigg \{
  			\begin{tabular}{ccc}
  			$1$ & if $s^t_{(j-1)c+k} \ge  \mbox{select}(w, (s^t_{(j-1)c+k'})_{1\le k' \le l})$ \\
  			$0$ & otherwise 
  		\end{tabular}
  		$
	...
	We now study the performance of the new retrieving rule on distinct problems of retrieving messages : first partially erased messages, then erroneous messages, finally partially erased messages when the network has been damaged.
	
	\section{Retrieval performance}	
	
	Messages will be considered as independent and identically distributed according to the uniform distribution.
