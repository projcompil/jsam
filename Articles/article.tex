...
\begin{document}
	...
	\subsection{Retrieving messages}	
		...
		\begin{algorithmic}[1]
		\Require{$v$ a binary array of length $n = c\cdot l$, $W$ the weight matrix representing the network}
...
%	\end{align*}	
	...
	\section{Maximum theoretical efficiency}
		...
		The entropy of the messages set can be approximated as $H(M) \approx m c \log_2 {l \choose a}$. From $d = 1 - \left (1- \left(\frac{a}{l}\right)^2 \right)^m$, we can deduce $m \sim \frac{\log_2(1-d)}{\log_2(1-(\frac{a}{l})^2)} \sim - \frac{l^2 \log_2(1-d) \log 2}{a^2}$.
